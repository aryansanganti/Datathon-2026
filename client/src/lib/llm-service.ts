import type { EmployeeData, LLMAllocationResponse, TaskData } from './types';
...
export const getEmployees = async (): Promise<EmployeeData[]> => {
  ...
  if (USE_MONGODB) {
    ...
    return cachedEmployees || syntheticEmployees;
  }
...
const _extractTechFromDescription = (description: string): string[] => {
  const techKeywords = [
...
const _autoGenerateTechStack = (featureName: string, description: string, employees: EmployeeData[]): string[] => {
...
const _generateFallbackAllocation = generateSmartFallbackAllocation;
